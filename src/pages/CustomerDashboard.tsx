...
const CustomerDashboard: React.FC = () => {
  ...
  const handleMenuClick = (event: React.MouseEvent<HTMLButtonElement>, iqamaId: string) => {
  ...
  const handleDelete = async () => {
    ...
    if (confirmDelete && selectedCustomerId) {
      try {
        await axios.delete(`${API_URL}/customers/${selectedCustomerId}`, {
          headers: {
            "Content-Type": "application/json",  // ⬅️ Explicit header
          },
        });
            ...
            {customers.map((customer) => (
              <TableRow key={customer.iqama_id} sx={{ '&:nth-of-type(odd)': { backgroundColor: '#f9f9f9' } }}>
                ...
                <TableCell align="right">
                  <IconButton onClick={(e) => handleMenuClick(e, customer.iqama_id)}>
                    <MoreVertIcon />
                  </IconButton>
                </TableCell>
              </TableRow>
